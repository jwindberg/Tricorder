        ...
        Row(modifier = Modifier.fillMaxSize()) {
            ...
            Column(modifier = Modifier.weight(1f).fillMaxHeight()) {
                 Box(
                    ...
                    LcarsDialAtom(
                        angle = alt, 
                        gridColor = gridColor, // Lavender Arc
                        plotColor = LcarsSourceColors.COL_RED_STRICT, // Red Pointer for Dial (Source: -65536)
        ...
        Row(modifier = Modifier.fillMaxSize()) {
            ...
            Box(
                ...
                LcarsChartGraph(
                    dataPoints = history,
                    gridColor = gridColor,
                    plotColor = plotColor, // Dynamic Plot Color
             ...
             Column(modifier = Modifier.width(40.dp).fillMaxHeight()) {
                 LcarsGaugeAtom(
                    ...
                    color = plotColor, // Uses Plot Color (Gold for MAG, Orange for GRA)
                    gridColor = gridColor, // Bar Color
    ...
    Column(modifier = modifier) {
        ...
        Row(modifier = Modifier.fillMaxSize()) {
            ...
            Column(modifier = Modifier.weight(1f)) {
                DataRow("AZI", az, plotColor)
                DataRow("ALT", alt, plotColor)
                DataRow("MAG", mag, plotColor)
            }
            
            // Right Bar
            Box(modifier = Modifier.width(SIDE_BAR_WIDTH).fillMaxHeight().background(gridColor))
        }
    }
}

@Composable
fun DataRow(label: String, value: Float, color: Color) {
    Row(modifier = Modifier.fillMaxWidth().padding(vertical = 2.dp)) {
        ...
        Text(
            ...
            color = color, // Dynamic Plot Color for values
            fontFamily = FontFamily.Monospace,
            fontSize = 16.sp
        )
    }
